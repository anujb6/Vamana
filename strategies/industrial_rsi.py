...
def compute(db_path='data/vamana.db'):
    """
    Compute industry-level price indices and RSI from individual stock data.

    Reads stock data from SQLite database, aggregates by industry using equal weighting,
    ...
    with DatabaseWriter(db_path) as db:
        ...
        industries = db.get_unique_industries()
        total_industries = len(industries)

        print(f"Computing industry indices for {total_industries} industries...")

        # Clear existing industry prices
        db.clear_industry_prices()

        for idx, industry in enumerate(industries):
            try:
                # Get all symbols in this industry
                symbols = db.get_symbols_by_industry(industry)

                if not symbols:
                    print(f"  [{idx + 1}/{total_industries}] {industry}: no symbols found, skipping")
                    continue
                ...
                # Load and normalize price data for each company in the industry
                ...
                if not valid_companies:
                    print(f"  [{idx + 1}/{total_industries}] {industry}: no valid data, skipping")
                    continue
                ...
                # Store in database
                records = db.insert_industry_prices(industry, monthly_price_df)

                print(f"  [{idx + 1}/{total_industries}] {industry}: {records} records saved ({total_companies} companies)")

            except Exception as e:
                print(f"  [{idx + 1}/{total_industries}] {industry}: Error - {e}")

        # Update metadata
        from datetime import datetime
        db.update_metadata('industry_rsi_last_updated', datetime.now().isoformat())

    print(f"\nIndustry RSI computation complete.")
