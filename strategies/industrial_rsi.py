    ...
    for industry, companies in industry_dict.items():
        ...
        if valid_companies:
            ...
            if len(monthly_price_df) <= 14:
                print(f"Warning: Industry {industry} has insufficient data for reliable RSI calculation")
            
            # Save the results
            ind = industry.lower().replace(' ', '_')
            industry_dir = f'data/industries/{ind}'
            os.makedirs(industry_dir, exist_ok=True)
            monthly_price_df.to_csv(f"{industry_dir}/{ind}_price.csv")
            chart_path = f"{industry_dir}/{ind}_chart.html"
            save_interactive_chart(ind, monthly_price_df, chart_path, f'{ind} Industry Index - Monthly')
            print(f"Processed industry: {ind} - Chart saved at {chart_path}")