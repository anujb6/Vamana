...
def compute(db_path='data/vamana.db'):
    """
    Compute basic industry-level price indices and RSI from individual stock data.

    Reads stock data from SQLite database, aggregates by basic industry using equal weighting,
    ...
    with DatabaseWriter(db_path) as db:
        ...
        basic_industries = db.get_unique_basic_industries()
        total_basic_industries = len(basic_industries)

        print(f"Computing basic industry indices for {total_basic_industries} basic industries...")

        # Clear existing basic industry prices
        db.clear_basic_industry_prices()

        for idx, basic_industry in enumerate(basic_industries):
            try:
                # Get all symbols in this basic industry
                symbols = db.get_symbols_by_basic_industry(basic_industry)

                if not symbols:
                    print(f"  [{idx + 1}/{total_basic_industries}] {basic_industry}: no symbols found, skipping")
                    continue
                ...
                # Load and normalize price data for each company in the basic industry
                ...
                if not valid_companies:
                    print(f"  [{idx + 1}/{total_basic_industries}] {basic_industry}: no valid data, skipping")
                    continue
                ...
                # Store in database
                records = db.insert_basic_industry_prices(basic_industry, monthly_price_df)

                print(f"  [{idx + 1}/{total_basic_industries}] {basic_industry}: {records} records saved ({total_companies} companies)")

            except Exception as e:
                print(f"  [{idx + 1}/{total_basic_industries}] {basic_industry}: Error - {e}")

        # Update metadata
        from datetime import datetime
        db.update_metadata('basic_industry_rsi_last_updated', datetime.now().isoformat())

    print(f"\nBasic Industry RSI computation complete.")
